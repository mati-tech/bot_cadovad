...
# ID администратора для уведомлений (замените на ваш реальный ID)
ADMIN_ID = 1237042422  # Замените на ваш Telegram ID
...
async def payment_status(callback: CallbackQuery):
    with SessionLocal() as session:
        ...
        # Клавиатура с тарифными планами в рублях
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(text="💰 1 месяц - 1500 ₽", callback_data="plan_1month")],
                [InlineKeyboardButton(text="💰 3 месяца - 3000 ₽", callback_data="plan_3months")],
                [InlineKeyboardButton(text="💰 6 месяцев - 5000 ₽", callback_data="plan_6months")],
                [InlineKeyboardButton(text="💰 1 год - 10000 ₽", callback_data="plan_1year")],
                [
                    InlineKeyboardButton(text="📱 Связаться с админом", callback_data="contact_admin"),
                    InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_settings")
                ]
...
async def select_plan(callback: CallbackQuery, state: FSMContext):
    # Карта тарифных планов в рублях
    plan_map = {
        "plan_1month": {"name": "1 месяц", "price": 1500.00, "days": 30},
        "plan_3months": {"name": "3 месяца", "price": 3000.00, "days": 90},
        "plan_6months": {"name": "6 месяцев", "price": 5000.00, "days": 180},
        "plan_1year": {"name": "1 год", "price": 10000.00, "days": 365}
    }
    ...
    # Сохраняем данные плана в состоянии
    await state.update_data(
        selected_plan=plan_data['name'],
        selected_price=plan_data['price'],
        selected_days=plan_data['days'],
        plan_key=callback.data
    )
    
    # Показываем реквизиты для оплаты
    bank_card_number = "2200 1111 2222 3333"  # Замените на ваш реальный номер карты
    
    payment_instructions = (
        f"💰 **Подписка на {plan_data['name']}**\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💵 Сумма к оплате: {plan_data['price']:.2f} ₽\n"
        f"📅 Длительность: {plan_data['days']} дней\n\n"
        f"💳 **Реквизиты для оплаты:**\n"
        f"Номер карты: `{bank_card_number}`\n\n"
        f"📝 **Инструкция:**\n"
        f"1. Переведите {plan_data['price']:.2f} ₽ на указанную карту\n"
        f"2. Сохраните скриншот или чек оплаты\n"
        f"3. Нажмите кнопку '✅ Я оплатил' ниже\n"
        f"4. Ожидайте подтверждения от администратора\n\n"
        f"⚠️ **Внимание:**\n"
        f"• Укажите в комментарии ваш ID: `{callback.from_user.id}`\n"
        f"• Подтверждение обычно занимает до 24 часов\n"
        f"• При проблемах свяжитесь с администратором"
    )
    ...
    await callback.message.answer(payment_instructions, reply_markup=keyboard)
    await callback.answer()
...
async def confirm_payment(callback: CallbackQuery, state: FSMContext):
    ...
    if not data:
        await callback.message.answer("❌ Ошибка: данные о платеже не найдены. Пожалуйста, выберите тариф снова.")
        await state.clear()
        await callback.answer()
        return
    
    plan_name = data.get('selected_plan', 'Неизвестный план')
    price = data.get('selected_price', 0)
    days = data.get('selected_days', 30)
    ...
    with SessionLocal() as session:
        ...
        # Создаем запись о платеже со статусом "ожидает подтверждения"
        payment = Payment(
            user_id=user.id,
            amount=price,
            plan_type=plan_name,
        ...
        admin_message = (
            ...
            f"📍 Магазин: {user.location or 'Не указан'}\n\n"
            f"💰 **Детали платежа:**\n"
            f"• Тариф: {plan_name}\n"
            f"• Сумма: {price:.2f} ₽\n"
            f"• Дней: {days}\n"
        ...
        try:
            ...
            # Если не удалось отправить, сохраняем в лог
        
        # Сообщение пользователю
        user_message = (
            f"✅ **Заявка на оплату отправлена!**\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Ваш платеж ожидает подтверждения администратором.\n\n"
            f"📋 **Детали:**\n"
            f"• Тариф: {plan_name}\n"
            f"• Сумма: {price:.2f} ₽\n"
            f"• ID платежа: {payment_id}\n\n"
            ...
            f"2. Он проверит поступление средств\n"
            f"3. Подтвердит ваш платеж\n"
            f"4. Ваша подписка будет активирована\n\n"
            f"Обычно это занимает до 24 часов.\n"
            f"Статус можно проверить в разделе '💳 Статус оплаты'."
        )
...
async def start_support_message(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(PaymentState.waiting_for_confirmation)  # Используем существующее состояние
    await callback.answer()

# Обработка сообщения в поддержку
@router.message(PaymentState.waiting_for_confirmation)
async def send_support_message(message: Message, state: FSMContext):
    if message.text == "/cancel":
        await message.answer("❌ Сообщение отменено.")
        await state.clear()
...
async def faq_section(callback: CallbackQuery):
    faq_text = (
        ...
        "👉 Настройки → Статус оплаты → Выберите тариф → Оплатите на карту → Нажмите 'Я оплатил'\n\n"
        "❓ Сколько ждать подтверждения оплаты?\n"
        "👉 Обычно до 24 часов. Администратор проверяет платежи вручную.\n\n"
        "❓ Как добавить товар?\n"
        "👉 Перейдите в Товары → Добавить новый товар\n\n"
        "❓ Как отметить продажу?\n"
        "👉 Нажмите на товар и выберите 'Отметить как проданный'\n\n"
